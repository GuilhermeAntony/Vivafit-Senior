...
export default function Exercises({ navigation }: Props) {
  ...
  const filtered = useMemo(() => {
    let list = selectedCategory === 'Todos' ? PREDEFINED_EXERCISES.slice() : PREDEFINED_EXERCISES.filter(e => e.category === selectedCategory);
    if (query.trim()) {
  ...
  const startExercise = (exercise: Exercise) => {
    // Usar o formato correto baseado na interface de navegação
  ...
  function CachedImage({ id, uri }: { id: string | number; uri?: string | null }) {
    const { localUri } = useCachedImage(id, uri || undefined);
    return (
      <Image
        source={localUri ? { uri: localUri } : (uri ? { uri } : require('../../assets/icon.png'))}
            ...
            <Card key={exercise.id} style={{ shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 8 }}>
              <CardHeader style={{ paddingBottom: 12 }}>
                <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
                  <CachedImage id={exercise.id} uri={exercise.imageUrl} />
                  <View style={{ flex: 1 }}>
